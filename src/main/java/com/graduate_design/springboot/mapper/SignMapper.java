...
public interface SignMapper {
    ...
    // 已经报名的活动列表
    @Select("SELECT * FROM sign WHERE userID = #{userID} AND activityName LIKE concat('%', #{activityName}, '%') " +
    ...
    @Select("SELECT count(*) FROM sign WHERE userID = #{userID} AND activityName LIKE concat('%', #{activityName}, '%') " +
            "AND address LIKE concat('%', #{address}, '%') AND organizer LIKE concat('%', #{organizer}, '%') ")
    Integer selectTotalSign(Integer userID, String activityName, String organizer, String address);

    // 已参加活动列表
    @Select("SELECT * FROM sign WHERE sign.participatingStatus = '1' AND sign.userID = #{userID} AND activityName LIKE " +
    ...
    @Select("SELECT count(*) FROM sign WHERE sign.participatingStatus = '1' AND sign.userID = #{userID} AND activityName " +
